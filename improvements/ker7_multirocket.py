...
def _fit_biases(X, dilations, num_features_per_dilation, quantiles):
    ...
    # >>> indices = np.array([_ for _ in combinations(np.arange(7), 3)], dtype = np.int32)
    indices = np.array((
    ...
    for dilation_index in range(num_dilations):
        ...
        for kernel_index in range(num_kernels):
            ...
            C_gamma = np.zeros((7, input_length), dtype=np.float32)
            C_gamma[7 // 2] = G

            start = dilation
            end = input_length - padding

            for gamma_index in range(7 // 2):
                C_alpha[-end:] = C_alpha[-end:] + A[:end]
                C_gamma[gamma_index, -end:] = G[:end]

                end += dilation

            for gamma_index in range(7 // 2 + 1, 7):
...
def _fit_dilations(input_length, num_features, max_dilations_per_kernel):
    ...
    max_exponent = np.log2((input_length - 1) / (7 - 1))
...
def transform(X, X1, parameters, parameters1, n_features_per_kernel=4):
    ...
    for example_index in prange(num_examples):
        ...
        for dilation_index in range(num_dilations1):
            ...
            C_gamma = np.zeros((7, input_length - 1), dtype=np.float32)
            C_gamma[7 // 2] = G1
