...
def transform(X, X1, parameters, parameters1, n_features_per_kernel=5): #4 -> 5
    num_examples, input_length = X.shape
            ...
            for kernel_index in range(num_kernels):
                ...
                if _padding1 == 0:
                    for feature_count in range(num_features_this_dilation):
                        ...
                        if ppv > 0:
                            mu = sum_x / ppv
                            var = (sum_squares / ppv) - mu ** 2
                            if var > 0:
                                skew = ((sum_cubes / ppv) - 3 * mu * (sum_squares / ppv) + 2 * mu ** 3) / (var ** 1.5)
                            else:
                                skew = 0
                        else:
                            skew = 0
                        ...
                        features[example_index, end] = skew if ppv > 0 else 0
                else:
...
class MultiRocket:
    ...
    def __init__(
        ...
        self.n_features_per_kernel = 5 #4 -> 5
